    ...
    public Object intercept(Object obj, Method method, Object[] args, MethodProxy proxy) throws Throwable {
        if (IApi.class.isAssignableFrom(obj.getClass())) {
            ...
            if (null != meta) {
                ...
                case POST:
                    response = RequestExecutor.post(targetUrl, entity, headerArray);
                    TEMPORARY_HEADERS.remove();
                    return response;
                case PUT:
                    response = RequestExecutor.put(targetUrl, entity, headerArray);
                    TEMPORARY_HEADERS.remove();
                    return response;
                case GET:
                    response = RequestExecutor.get(targetUrl, headerArray);
                    TEMPORARY_HEADERS.remove();
                    return response;
                case DELETE:
                    response = RequestExecutor.delete(targetUrl, headerArray);
                    TEMPORARY_HEADERS.remove();
                    return response;
                default:
                    break;
